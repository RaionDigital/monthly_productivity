...
def execute(filters=None):
	...
	if view_mode == "Detailed Invoice View":
		...
		if not filters.get("month") or not filters.get("year"):
			frappe.msgprint(_("Please select a Month and Year for the Detailed View."), indicator='orange', title=_('Filter Required'))
			return [], [], None, None, None
...
# --- Functions for Monthly Detailed View ---

def get_monthly_details_columns():
	# --- MODIFIED: Columns are updated as per your request ---
	return [
		...
		{"label": _("Execution %"), "fieldname": "execution_percentage", "fieldtype": "Percent", "width": 120},
		{"label": _("Cumulative %"), "fieldname": "cumulative_execution", "fieldtype": "Percent", "width": 120},
		{"label": _("Commission"), "fieldname": "commission", "fieldtype": "Currency", "width": 150},
	]

def get_monthly_details_data(filters):
	# --- MODIFIED: Logic is updated for the new column requirements ---
	sql_filters = {"company": filters.get("company"), "month": filters.get("month"), "year": filters.get("year")}
	
	# Get all individual productivity entries for the selected month and year, including all necessary fields.
	invoice_details = frappe.db.sql("""
		...
		WHERE mp.docstatus = 1 AND mp.company = %(company)s 
		AND MONTH(mp.report_month) = %(month)s AND YEAR(mp.report_month) = %(year)s
	...
	for row in invoice_details:
		# For this view, profit is simply the executed value, as we are ignoring costs at the row level.
		profit = row.executed_value or 0
		commission_percent = row.commission_percentage or 0
		row["commission"] = profit * (commission_percent / 100.0)

	# The total row is no longer needed for this view.
	return invoice_details

# --- Functions for Invoice Progress View (Unchanged) ---
def get_invoice_progress_columns(): return [{"label": _("Month"), "fieldname": "month", "fieldtype": "Data", "width": 200}, {"label": _("Execution % This Period"), "fieldname": "execution_percentage", "fieldtype": "Percent", "width": 200}, {"label": _("Cumulative Execution %"), "fieldname": "cumulative_execution", "fieldtype": "Percent", "width": 200}, {"label": _("Monthly Productivity Doc"), "fieldname": "monthly_productivity_doc", "fieldtype": "Link", "options": "Monthly Productivity", "width": 200}]
def get_invoice_progress_data(filters):
	progress_entries = frappe.db.sql("SELECT mp.name as monthly_productivity_doc, mp.report_month, ese.execution_percentage, ese.cumulative_execution FROM `tabExecution Schedule Entry` as ese JOIN `tabMonthly Productivity` as mp ON ese.parent = mp.name WHERE ese.sales_invoice = %(sales_invoice)s AND mp.docstatus = 1 ORDER BY mp.report_month ASC", filters, as_dict=1)
	report_data = []
...
# --- Functions for Summary View (Unchanged) ---
def get_summary_columns(is_yearly_view):
	period_label = _("Year") if is_yearly_view else _("Month")
	return [{"label": period_label, "fieldname": "period", "fieldtype": "Data", "width": 180}, {"label": _("Executed Value"), "fieldname": "executed_value", "fieldtype": "Currency", "width": 150}, {"label": _("Total Purchases"), "fieldname": "total_purchases", "fieldtype": "Currency", "width": 150}, {"label": _("Other Expenses"), "fieldname": "other_expenses", "fieldtype": "Currency", "width": 150}, {"label": _("Profit or Loss"), "fieldname": "profit_loss", "fieldtype": "Currency", "width": 150}, {"label": _("Commission"), "fieldname": "commission", "fieldtype": "Currency", "width": 150}]
def get_summary_data(filters, is_yearly_view):
	date_format = "'%%Y'" if is_yearly_view else "'%%Y-%%m'"
	sql_filters = {"company": filters.get("company"), "from_date": filters.get("from_date"), "to_date": filters.get("to_date")}
	executed_value_data = frappe.db.sql(f"SELECT DATE_FORMAT(mp.report_month, {date_format}) AS period_group, SUM(ese.actual_executed_value) AS total_executed_value, mp.commission_percentage FROM `tabMonthly Productivity` mp JOIN `tabExecution Schedule Entry` ese ON mp.name = ese.parent WHERE mp.docstatus = 1 AND mp.company = %(company)s AND mp.report_month BETWEEN %(from_date)s AND %(to_date)s GROUP BY period_group, mp.commission_percentage", sql_filters, as_dict=1)
	purchases_data = frappe.db.sql(f"SELECT DATE_FORMAT(pi.posting_date, {date_format}) as period_group, SUM(pi.base_grand_total) as total_purchases FROM `tabPurchase Invoice` pi WHERE pi.docstatus = 1 AND pi.company = %(company)s AND pi.posting_date BETWEEN %(from_date)s AND %(to_date)s GROUP BY period_group", sql_filters, as_dict=1)
	other_expenses_data = frappe.db.sql(f"SELECT DATE_FORMAT(je.posting_date, {date_format}) as period_group, SUM(jea.debit_in_account_currency) as total_other_expenses FROM `tabJournal Entry Account` jea JOIN `tabJournal Entry` je ON je.name = jea.parent WHERE je.docstatus = 1 AND je.company = %(company)s AND je.posting_date BETWEEN %(from_date)s AND %(to_date)s AND LEFT(jea.account, 2) IN ('62', '63', '64', '65', '66', '67', '68', '69') GROUP BY period_group", sql_filters, as_dict=1)
	period_summary = {}
